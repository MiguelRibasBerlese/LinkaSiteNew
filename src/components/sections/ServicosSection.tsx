  ...
  return (
    <section id="servicos" className="relative z-[2] mx-auto max-w-6xl px-6 pb-40 pt-16 md:px-12">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-6">
        {SERVICES.map((svc, i) => {
          const cardClass = `group relative flex min-h-[220px] flex-col justify-between rounded-2xl border border-brand-primary/40 bg-brand-panel p-9 transition-all duration-200 hover:border-brand-primary hover:shadow-[0_0_40px_-12px_rgba(133,71,228,0.45)] ${svc.span}`
          const cardContent = (
            <>
          ...
          // ponytail: skip framer-motion's whileInView fade+scale on mobile, it's another scroll-driven
          // reflow that stacks on top of the ZoomParallax fix and adds to the same delay
          if (isMobile) {
            return (
